<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="transition mt-6 space-y-6"
        ...
        <div>
            ...
            <textarea autofocus id="bio" name="bio"
        ...
        <div class="flex items-center gap-4">
            ...
            @if (session('status') === 'profile-updated')
                <p x-data="{ show: true }" x-show="show" x-transition x-init="setTimeout(() => show = false, 3000)"
                    class=" grow text-sm text-green-700 dark:text-gray-400 border border-green-700 rounded font-bold bg-green-100 py-1 text-center">
                    {{ __('Saved.') }}</p>
            @endif
        </div>
    </form>
